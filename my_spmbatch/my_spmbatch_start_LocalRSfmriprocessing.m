    ...
    if params.use_parallel && ~params.onVSC  && params.run_background
        for j=1:numpacks
            ...
            for is = 1:maxruns
                ...
                logfile{i} = fullfile(datpath,['localrsfmri_process_logfile_' sprintf(['%0' num2str(params.sub_digits) 'd'],datlist(i,1)) '_' sprintf('%02d',datlist(i,2)) '_' sprintf('%02d',datlist(i,3)) '_' params.task{k} '.txt']);
        
                if exist(logfile{i},'file'), delete(logfile{i}); end
                
                if ispc
                    mtlb_cmd = sprintf("restoredefaultpath;addpath(genpath('%s'));addpath(genpath('%s'));my_spmbatch_run_localrsfmriprocessing(%d,%d,%d,'%s','%s','%s');exit", ...
                ...
                for is = 1:maxruns
                    ...
                    if exist(logfile{i},'file')
                        ...
                        if ~isempty(errortest)
                            ...
                            t = datetime('now','TimeZone','local','Format','d-MMM-y HH:mm:ss');
                            nlogfname = fullfile(datpath,['error_localrsfmri_process_logfile_' sprintf(['%0' num2str(params.sub_digits) 'd'],datlist(i,1)) '_' sprintf('%02d',datlist(i,2)) '_' sprintf('%02d',datlist(i,3)) '_' params.task{k} '.txt']);
                        ...
                        elseif ~isempty(test)
                            ...
                            if ~params.keeplogs
                                ...
                                nlogfname = fullfile(datpath,['done_localrsfmri_process_logfile_' sprintf(['%0' num2str(params.sub_digits) 'd'],datlist(i,1)) '_' sprintf('%02d',datlist(i,2)) '_' sprintf('%02d',datlist(i,3)) '_' params.task{k} '.txt']);
                                movefile(logfile{i},nlogfname);
                            end
