...
%+------------------------------------------------------------------------+
%| Copyright: Manuel Morante Moreno                                       |
...
%|        spatial map with values ?? [0,100]                              |
%|        NOTE: 0% Means that the spatial maps is dense (no zeros)        |
%|                                                                        |
%|   === OPTIONAL PARAMETERS ==========================================   |
...
%--------------------------------------------------------------------------
function [D,S,E] = tedm_IADL(param) % Main Function
...
% === INITIALIZATION ======================================================
    ...
    if(Verb);Pbar = waitbar(0,'Initialization','Name','IADL');end
...
% === CANONICAL DICTIONARY (Safe) =========================================
	if(isfield(param,'Del'))

		if(Verb); fprintf('   - Setting Ref. Dict. ---- [  ]'); end
		if(Verb); waitbar(0,Pbar,'Setting. Ref. Dict.');end
		...
		if(Verb); waitbar(i*1./M,Pbar);end
		if(Verb); fprintf('\b\b\bOk]\n'); end
...
% ======== ORDERING =======================================================
	...
	if(Verb); waitbar(0,Pbar,'Ordering');end
	% Initialization
	...
	while(Es>=Elim)&&(cnt<=Lit)
		...
		if(Verb);waitbar(max(0,Err*1./100),Pbar);end
	end
...
% === MAIN LOOP ===========================================================
	...
	if(Verb);waitbar(0,Pbar,'Progress','Name','IADL');end
...
% === POST-PROCESSING =====================================================
	...
	if(Verb);fprintf('\n'); end
	if(Verb);waitbar(0,Pbar,'Post-processing');end
...
function [S] = NewCoef(So,D,param)
	...
	for t = 1:Ts
		...
		for i = iS(cnd)
			A(i,:) = tedm_DuWProjectOpt(A(i,:),W(i,:),Lam(i));
		end
...
function [S] = NewCoefUni(So,D,param)
	...
	for t = 1:Ts
		...
		if (cnd)
			a = tedm_DuWProjectOpt(a,w,sum(Lam));
